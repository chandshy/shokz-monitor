...
# Seconds between successive reconnect attempts (last value repeats).
...
class BlueZMonitor:
    ...
    def _absorb(self, path: str, ifaces: dict) -> None:
        ...
        if _IFACE_BAT in ifaces:
            ...
            # Device appeared as connected but no Battery1 — flag it.
    ...
    def _on_props_changed(
        ...
        if interface == _IFACE_DEV:
            ...
            if "Percentage" in changed:
                ...
                self._state.battery = int(pct) if pct is not None else None
                self._state.battery_interface_missing = False
                updated = True
    ...
    def _on_connected(self) -> None:
        ...
        # BlueZ sometimes registers Battery1 a few seconds after connect.
        if self._battery_timer is not None:
            GLib.source_remove(self._battery_timer)
        self._battery_timer = GLib.timeout_add_seconds(5, self._refresh_battery)
    ...
    def _do_reconnect(self) -> bool:
        ...
        try:
            ...
            log.debug("Reconnect %d failed: %s", self._reconnect_attempt, exc)
            if self._reconnect_attempt < 10:
                self._schedule_reconnect()
        return False  # one-shot

    def _reconnect_error(self, error: dbus.DBusException) -> None:
        log.debug("Async reconnect error: %s", error)
        if not self._state.connected and self._reconnect_attempt < 10:
            self._schedule_reconnect()
